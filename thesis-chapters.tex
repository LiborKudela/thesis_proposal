...
\label{struktura}
V této kapitole je provedena rešerše literatury zabývající se
problematikou modelování CZT, kterou v  a popis vybraných matematických rovnic
aplikovatelných pro daný kontext. Dále jsou popsány použitelné výpočetní
přístupy metody spolu s aspekty, ovlivňující numerickou efektivnost simulací.
...
\begin{itemize}
  ...
  \item
    \textbf{Axiální difúze} je složena jednak z difúze kodkukcí tepla v
    ...
    dle přibližně lineárně s Reynoldsovým číslem \cite{Chertkov2018}. Axiální
...
NN pro matematické modelování ODR systémů (a tudíž i PDR)
...
gradient byl určen algoritmickou zpětnou propagací zkrze ODR solver (což je tím
největším přínosem těchto prací). Gradient descend je obecně považován za
nejvíce pokročilou metodu učení modelů avšak je poměrně náročný na
implementaci.

Existují i algoritmy jež nevyžadují derivace a jsou vhodné pro modely u kterých
nelze derivace určit. Tyto algoritmy umožňují optimalizaci až na úrovni black
box.

Machine learning a mechanistické modelování na základě PDR lze tedy kombinovat.
Tímto je možné modelovat procesy na základě experimentálních dat nebo
zjednodušovat komplexnost programu na základě extrahovaných veličin z více
komplexních simulací.
