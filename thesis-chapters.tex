...
neuronových modelů. Cílovými veličinami mohou být experimentální data, ale také
data z komplexnějších simulací nebo penalizační účelové funkce (například v
případě optimalizace řídících veličin).
...
\begin{equation}
  ...
  \end{gathered}
\end{equation}
kde \(\Theta \in \left[0,1\right]\) umožňuje řídit implicitnost metody
(pro \(\Omega = \frac{1}{2}\) se jedná o \mbox{Crank-Nicolsonovu}) s časovým
krokem \(\Delta{t}\). Funkce \(\phi_i\) a \(\hat{\phi_j}\) jsou tzv. test a
trial funkce.

Na základě dat z těchto simulací bude chování modelu přeneseno optimalizací
parametrů (pomocí LOFI) do zjednodušeného ekvivalentního schématu
naprogramovaného v jazyce Modelica. Změnou topologie schématu je možné měnit
chování modelu (přesnost a numerická efektivita). Tyto modely akumulace budou
schopny propojení s modelem fluidního regionu i vlivy okolí.
...
komponent bude do modelu zahrnuta časová distribuce řídících veličin jako jsou
