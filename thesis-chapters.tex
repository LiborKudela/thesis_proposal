...
programovací jazyky ve kterých programátor definuje logické a matematické
...
OpenModelica \todo{cite OM} je open-source verze kompilátoru, který překládá
...
složeného z těchto komponent se zaměřením na CZT. V principu se jde o
modelování PDR. Jedná se tedy o základní výzkum zaměřený na optimalizaci
numerických výpočtů/simulací. Jeho řešení spočívá ve vytvoření optimalizačního
nástroje s aplikačními rozhraními umožňující jeho propojení s matematickými
modely/simulacemi implementovanými v různých programovacích jazycích.
\section{Cíle práce}
\label{sec:section name}
Cílem disertační práce je vytvořit a propojit nástroje pro optimalizaci
parametrizovaných programů reprezentující matematické modely komponent a
systémů a demonstrovat jeho funkčnost pro modely CZT. Optimalizační nástroj
bude schopen optimalizovat parametry jak mechanistických tak neuronových
modelů. Cílovými veličinami mohou být jak experimentální data, ale také data z
komplexnějších simulací nebo penalizační účelové funkce (například v případě
optimalizace řídících veličin). 
\begin{itemize}
  \item
  \item
\end{itemize}
